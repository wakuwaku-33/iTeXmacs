...
(kbd-map
  ...
  ("F2" (choose-file load-buffer "Open" ""))
  ("S-F2" (choose-file load-in-new-window "Open in new window" ""))
  ("C-F2" (revert-buffer))
  ("M-F2" (new-buffer))
  ("M-S-F2" (open-window))
  ("M-C-F2" (clone-window))
  ("F3" (if (no-name?) (choose-file save-buffer "Save" "texmacs") (save-buffer)))
  ("S-F3" (choose-file save-buffer "Save as" "texmacs"))
