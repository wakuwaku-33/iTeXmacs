...
Source: http://itexmacs.googlecode.com/files/iTeXmacs-1.0.7.6.tgz
License: GNU GPL 3.0
...
BuildRoot: %{_tmppath}/iTeXmacs-1.0.7.6-root

%description

iTeXmacs, which is based on GNU TeXmacs, is a free wysiwyw (what you see is what
you want) editing platform with special features for scientists. The software 
...
a laptop. Moreover, iTeXmacs can be used as an interface for many external
...
iTeXmacs runs on all major Unix platforms and Windows. Documents can be
saved in iTeXmacs, Xml or Scheme format and printed as Postscript or
Pdf files. Converters exist for TeX/LaTeX and Html/Mathml.

%prep
%setup -q -n iTeXmacs-1.0.7.6-src
